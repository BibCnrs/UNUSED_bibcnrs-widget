...
export const BibHeader = ({ EBSCO_WIDGET_domain, EBSCO_WIDGET_username, EBSCO_WIDGET_availableDomains, language }) => {
    ...
    return (
        <div className="connexion bsbb">
            <a href="/">
                <img
                    src={process.env.NODE_ENV === 'test' ? `lib/images/bibcnrs-logo-${EBSCO_WIDGET_domain || 'visite'}.png` : require(`../images/bibcnrs-logo-${EBSCO_WIDGET_domain || 'visite'}.png`)}
                    className="bibcnrslogo"
                    alt={'logo bibcnrs ' + EBSCO_WIDGET_domain}
                />
            </a>
            ...
            <div className="infos bsbb">
                ...
                {EBSCO_WIDGET_availableDomains ? <span>{translation[language].authorizations} : {EBSCO_WIDGET_availableDomains.map(value => translation[language][value]).join(', ')}</span> : null}</div>
            </div>
        </div>
    );
};
